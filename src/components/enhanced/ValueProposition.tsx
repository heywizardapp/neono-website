import { IntersectionAnimation } from '@/components/advanced/IntersectionAnimations';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { CheckCircle, ArrowRight, Zap, Clock, DollarSign } from 'lucide-react';
import { Link } from 'react-router-dom';

export function ValueProposition() {
  const benefits = [
    {
      icon: Zap,
      title: "Setup in minutes",
      description: "Go live today with zero technical knowledge required"
    },
    {
      icon: Clock,
      title: "Save 10+ hours/week",
      description: "Automate scheduling, payments, and customer communications"
    },
    {
      icon: DollarSign,
      title: "Increase revenue 25%",
      description: "Smart booking, upselling, and retention tools that work"
    }
  ];

  const features = [
    "Online booking & calendar management",
    "Point of sale with tip splitting",
    "SMS & email marketing automation", 
    "Staff management & payroll",
    "Customer profiles & history",
    "Real-time analytics & reporting",
    "Website & social media integration",
    "24/7 customer support"
  ];

  return (
    <section className="py-12 lg:py-16">
      <div className="container">
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
          {/* Left side - Value props */}
          <div>
            <IntersectionAnimation animation="fade-up">
              <Badge variant="secondary" className="mb-6">
                Why NeonO?
              </Badge>
              
              <h2 className="text-3xl font-bold mb-6 lg:text-4xl">
                Everything you need to run your business, in one place
              </h2>
              
              <p className="text-xl text-muted-foreground mb-8 leading-relaxed">
                Stop juggling multiple apps. NeonO brings appointments, payments, marketing, 
                and analytics together in one beautiful, easy-to-use platform.
              </p>
            </IntersectionAnimation>

            <div className="space-y-6 mb-8">
              {benefits.map((benefit, index) => {
                const Icon = benefit.icon;
                return (
                  <IntersectionAnimation 
                    key={benefit.title}
                    animation="fade-up" 
                    delay={index * 100}
                  >
                    <div className="flex gap-4 items-start">
                      <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
                        <Icon className="w-6 h-6 text-primary" />
                      </div>
                      <div>
                        <h3 className="font-semibold mb-2">{benefit.title}</h3>
                        <p className="text-muted-foreground">{benefit.description}</p>
                      </div>
                    </div>
                  </IntersectionAnimation>
                );
              })}
            </div>

            <IntersectionAnimation animation="fade-up" delay={400}>
              <Button size="lg" className="group" asChild>
                <Link to="/signup">
                  Start Your Free Trial
                  <ArrowRight className="ml-2 h-5 w-5 transition-transform group-hover:translate-x-1" />
                </Link>
              </Button>
            </IntersectionAnimation>
          </div>

          {/* Right side - Feature checklist */}
          <div>
            <IntersectionAnimation animation="fade-up" delay={200}>
              <div className="bg-gradient-to-br from-white to-slate-50 rounded-2xl p-8 shadow-xl border border-border/50">
                <h3 className="text-xl font-bold mb-6">What's included:</h3>
                
                <div className="grid gap-3">
                  {features.map((feature, index) => (
                    <IntersectionAnimation 
                      key={feature}
                      animation="fade-up" 
                      delay={300 + index * 50}
                    >
                      <div className="flex items-center gap-3">
                        <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0" />
                        <span className="text-sm">{feature}</span>
                      </div>
                    </IntersectionAnimation>
                  ))}
                </div>

                <div className="mt-8 p-4 bg-primary/5 rounded-xl border border-primary/10">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium">Setup Progress</span>
                    <span className="text-sm text-muted-foreground">5 minutes</span>
                  </div>
                  <div className="h-2 bg-primary/20 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-primary rounded-full transition-all duration-1000"
                      style={{ width: '75%' }}
                    />
                  </div>
                  <p className="text-xs text-muted-foreground mt-2">
                    Average time to go live with NeonO
                  </p>
                </div>
              </div>
            </IntersectionAnimation>
          </div>
        </div>
      </div>
    </section>
  );
}